...
RegisterNetEvent('amari-jewel:Server:TriggerNotification')
AddEventHandler('amari-jewel:Server:TriggerNotification', function(playerId, msg, type, length)
    TriggerClientEvent('amari-jewel:Client:CustomNotify', playerId, msg, type, length)
end)


RegisterServerEvent('amari-jewel:server:Craft')
AddEventHandler('amari-jewel:server:Craft', function(itemHash, materials)
    local src = source
...
RegisterServerEvent('amari-jewel:Server:Removemats')
AddEventHandler('amari-jewel:Server:Removemats', function(item, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
...
QBCore.Functions.CreateCallback('amari-jewel:server:Materials', function(source, cb, materials)
    local src = source
    local item = 0
...
QBCore.Functions.CreateCallback('amari-jewel:server:HasItem', function(source, cb, itemKey)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player then
...
RegisterNetEvent("amari-jewel:Server:Billing", function(playerId, amount)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerEvent('amari-jewel:Server:TriggerNotification', source, Language.Notify.Send, 'success', 5000)
                    TriggerEvent('amari-jewel:Server:TriggerNotification', billed.PlayerData.source, Language.Notify.InvoiceReceived, 'info', 5000) -- Assuming 'info' is the type
                else
                    TriggerEvent('amari-jewel:Server:TriggerNotification', source, Language.Notify.HigherValue, 'error', 5000)
                end
            else
                TriggerEvent('amari-jewel:Server:TriggerNotification', source, Language.Notify.InvoiceOwn, 'error', 5000)
            end
        else
            TriggerEvent('amari-jewel:Server:TriggerNotification', source, Language.Notify.PlayerUnavailable, 'error', 5000)
        end
    else
        TriggerEvent('amari-jewel:Server:TriggerNotification', source, Language.Notify.NoPermission, 'error', 5000)
    end
end)

RegisterServerEvent('amari-jewel:server:BreakItem')
AddEventHandler('amari-jewel:server:BreakItem', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('amari-jewel:server:HasMarkedBills', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('amari-jewel:server:WashMoney')
AddEventHandler('amari-jewel:server:WashMoney', function(amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local markedBills = Player.Functions.GetItemByName("markedbills")
